...
# the width and height of the frame (w, h), and the chosen ML model.
...
def fallen(start_point, end_point, w, h, model_f):
    ...
    d = {'AR': AR, 'NW':NW, 'NH':NH, 'NB':NB, 'DIAG':DIAG, 'AREA':AREA}
    df_data = pd.DataFrame(data=d, index=[0])
    
    '''# The appropriate variables should be choosen for the corresponding MLP.'''
    # data = df_data[['AR','NW', 'NB']] # Dtree(1), SVM(1), Dtree(4), SVM(4)
    # data = df_data[['AR','NH', 'NB']] # Dtree(2), SVM(2), Dtree(5), SVM(5)
    data = df_data[['AR','NW', 'NB', 'DIAG']] # Dtree(3), SVM(3), Dtree(6), SVM(6)

    predict = model_f.predict(data)
    class_probabilities = model_f.predict_proba(data)
    # conf = max(model_fall.predict_proba(data)[0][0], model_fall.predict_proba(data)[0][1])
    # print(conf)
    conf = 0.8
    pred = predict.item()
    
    return pred, conf, AR, NW, NH, DIAG
    
'''# Check if a GPU is available.'''
device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")

'''# Give the math to the ML (.sav) model'''
model_fall = pickle.load(open('path to ML model', 'rb'))
